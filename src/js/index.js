(function() {
    ...
    function getDerivationPath() {
        if (bip44TabSelected()) {
            ...
            var coin = parseIntNoNaN(DOM.bip44coin.val(), 339);
    ...
    function TableRow(index, isLast) {
        ...
        function calculateValues() {
            setTimeout(function() {
                ...
                var privkey = "NA";
                if (hasPrivkey) {
                    //privkey = keyPair.toSecretCashAddress()
                ...
                if (networks[DOM.network.val()].name == "DVT - Devault") {
                    var bchAddrType = DOM.bitcoinCashAddressType.filter(":checked").val();
                    address = bchaddr.toCashAddress(address);
                }
                
                if (networks[DOM.network.val()].name == "DVT - Devault Testnet") {
                    var bchAddrType = DOM.bitcoinCashAddressType.filter(":checked").val();
                    address = bchaddr.toCashAddress(address);
                }
